...
def Subscribe(stream_id, option):
    ...
    if option == "lldp":
        ...
        request = sdk_service_pb2.NotificationRegisterRequest(
            op=op, stream_id=stream_id, lldp_neighbor=entry
        )
    elif option == "cfg":
...
def Announce_LLDP_using_EVPN(state, chassis_mac, portlist, is_port=False):
    logging.info(
        f"Announce_LLDP_using_EVPN({state.evpn_auto_lags}) :: {portlist}={chassis_mac}"
    )
    ...
    gnmiConnection(
        lambda c: c.set_with_retry(
            encoding="json_ietf", update=updates, delete=deletes
        ),
        note="Announce_LLDP_using_EVPN",
    )
...
class EVPNRouteMonitoringThread(threading.Thread):
    ...
    def run(self):
        ...
        gnmiConnection(
            lambda c: CreateEVPNCommunicationVRF(self.state, c), note="Create EVPN VRF"
        )
        ...
        try:
            for m in telemetry_stream:
                if m.HasField("update"):  # both update and delete events
                    ...
                    logging.info(
                        f"EVPNRouteMonitoringThread gNMI change event :: {parsed}"
                    )
                    ...
                    if update["update"]:
                        ...
                        if self.state.evpn_auto_lags == "large_communities":
                            ...
                            logging.error(
                                f"Unexpected value: {self.state.evpn_auto_lags}"
                            )
                        ...
                        for n2 in comms["notification"]:
                            if "update" in n2:  # Update is empty when path is invalid
                                for u3 in n2["update"]:
                                    ...
                                    lldp_ports = u3["val"]["attr-set"][0][
                                        "communities"
                                    ]["large-community"]
                                    logging.info(
                                        f"LLDP Communities from {peer_id}: {sorted(lldp_ports)}"
                                    )
                                    for p in lldp_ports:
                                        parts = p.split(":")
                                        key = (
                                            parts[1] + ":" + parts[2]
                                        )  # 48-bit MAC in 2 parts
                                        ...
                                        if key in self.state.local_lldp:
                                            (
                                                lag_port,
                                                provisioned,
                                            ) = self.state.local_lldp[key]
                                            logging.info(
                                                f"Found MC-LAG port match: {lag_port} peer={peer_id} provisioned={provisioned}"
                                            )
                                            if not provisioned:
                                                try:
                                                    # XXX only supports single port
                                                    Convert_lag_to_mc_lag(
                                                        self.state,
                                                        mac,
                                                        lag_port,
                                                        peer_id,
                                                    ...
                                                    logging.error(
                                                        f"Convert_lag_to_mc_lag BUG: {ex}"
                                                    )
            ...
            if "update" in n:  # Update is empty when path is invalid
                for u2 in n["update"]:
                    ...
                    for route in u2["val"]["ip-prefix-routes"]:
                        ...
                        encoded_lldp = route["ip-prefix"].split("/")[
                            0
                        ]  # /128 loopback IP
                        encoded_parts = encoded_lldp.split(":")  # XX can have '::'
                        mac = ":".join(
                            [
                                f"{(i>>8):02X}:{(i&0xff):02X}"
                        ...
                        if (
                            mac in self.state.local_lldp and encoded_parts[0] == "fda1"
                        ):  # 0xfda1 prefix for LAG macs (as opposed to DHCP macs)
                            ...
                            if not provisioned:
                                ...
                                try:
                                    ...
                                    logging.error(
                                        f"checkIPv6Routes: {ex} ~ {traceback_str}"
                                    )
                            ...
                            self.state.is_spine()
                            and self.state.dhcp
                            and encoded_parts[0] == "fdad"
                        ):
                            AddDHCPClient(
                                mac, peer_id, peer_ports, self.state, gnmi_client
                            )
...
def Set_LLDP_Systemname(state, name):
    ...
    gnmiConnection(
        lambda c: c.set_with_retry(
            encoding="json_ietf", update=[("/system/name", value)]
        )
    )
...
def Announce_LLDP_peer(state, name, port):
    logging.info(
        f"Announce_LLDP_peer :: name={name} port={port} ann={state.announcing}"
    )
...
def HandleLLDPChange(state, peername, my_port, their_port):
    ...
    if m:
        ...
        logging.info(
            f"HandleLLDPChange :: on={my_port} leaf={peer_ip}:{peer_if} name={peername} ann={state.announcing}"
        )
        ...
        logging.info(
            f"HandleLLDPChange :: no match on={my_port} name={peername} ann={state.announcing} pending={state.pending_announcements}"
        )
        if state.is_spine():
            if state.announcing == my_port:
                logging.info(
                    f"TODO check if not ACK, or ACK matching our systemname: {peername}"
                )
                if state.pending_announcements != []:
...
def Convert_to_lag(state, port, peer_data):
    ...
    use_irb = (
        state.useIRB() and peer_data["type"] == "host"
    )  # and not state.is_spine() ?
    ...
    if state.evpn_auto_lags != "disabled":
        ...
        updates = [
            (f"/interface[name=ethernet-1/{port}]/ethernet", {"aggregate-id": _lag})
        ]
        if (
            peer_data["type"] == "leaf" and not peer_data["pair"]
        ):  # leaf-leaf and leaf-spine, exclude same pair links
            ...
            if pair_key:
                ...
                if _pk in state.leaf_pairs:
                    ...
                    if _ab in pair and pair[_ab]["local"] != port:
                        logging.info(
                            "Detected self-loop link(s), not creating LAG for now"
                        )
                        if "self" in pair[_ab]:
                            pair[_ab]["self"] += [
                                {"local": port, "remote": peer_data["port"]}
                            ]
                        else:
                            pair[_ab]["self"] = [
                                {"local": port, "remote": peer_data["port"]}
                        ...
                        if (
                            len(pair) == 2 and int(_pk) != state.id_from_hostname
                        ):  # XXX Could have up to 4 ports
                            # mc_lag = True
                            ...
                            lag_desc = (
                                f"Leaf pair mc-lag on ports {pair['a']},{pair['b']}"
                            )
                            logging.info(
                                f"Convert_to_lag: Completed leaf-pair {pair} using {_lag}"
                            )
                            deletes += deletes_for_port(
                                _port_b if port == _port_a else _port_a
                            )
                            updates = [
                                (
                            ...
                            logging.warning(
                                f"Convert_to_lag: Still missing 2nd port? {pair}"
                            )
                            return None
                else:
                    state.leaf_pairs[_pk] = {_ab: port}
                    logging.info(
                        f"Convert_to_lag: Defer {port} until 2nd MC-LAG port is known"
                    )
                    return None
            else:
    ...
    is_routed = (
        state.get_role() == "leaf" and state.evpn != "l2_only_leaves"
    ) or state.is_spine()
    ...
    if use_vlans:
        lag["subinterface"][0]["srl_nokia-interfaces-vlans:vlan"] = {
            ...
            "encap": {"single-tagged": {"vlan-id": 4094}}
            if peer_data["type"] != "host" or is_routed
            else {"untagged": {}}
        }
    ...
    try:
        gnmiConnection(
            lambda c: c.set_with_retry(
                encoding="json_ietf", delete=deletes, update=updates
            )
        )
    except Exception as ex:
...
def Configure_EVPN(state, port, interface, ip):
        ...
        "subinterface": [
            {
                ...
                "admin-state": "enable",
                "ipv4": {
                    "address": [{"ip-prefix": f"{state.router_id}/32"}],
                    "admin-state": "enable",
                },
                "ipv6": {
                    "address": [
                        {"ip-prefix": f"2001::{state.router_id.replace('.',':')}/128"}
                    ],
                    "admin-state": "enable",
                },
            }
    ...
    if state.evpn == "symmetric_irb":
        ...
        ip_vrf["protocols"] = {
            ...
            "bgp-vpn": {
                "bgp-instance": [
                    {"id": 1, "_annotate": "Required to make bgp-evpn oper-state=up"}
                ]
            },
        }
    else:
        ip_vrf[
            "_annotate"
        ] = "This is asymmetric IRB, no BGP-EVPN or vxlan interface in this ip-vrf"
...
def Update_EVPN_RR_Neighbors(state, first_time=False):
    ...
    try:
        ...
        # Dont quit agent. Usually means 'overlay' didn't get created
        logging.error(
            f"Exception in Update_EVPN_RR_Neighbors gNMI set: {ex} state={state}"
        )
...
def Convert_lag_to_mc_lag(state, mac, port, peer_leaf, peer_port_list, gnmi_client):
    logging.info(
        f"Convert_lag_to_mc_lag :: port={port} mac={mac} peer_leaf={peer_leaf} peer_port_list={peer_port_list} leaf_pairs={state.leaf_pairs}"
    )
    ...
    if len(mc_lag) > 3:
        logging.error(
            f"Platform does not support MC-LAG with more than 4 members: {mc_lag}"
        )
        return False
                        ...
                        "ethernet-segment": [
                            {
                                ...
                                "_annotate_esi": "EVPN MC-LAG with "
                                + peers
                                + ", bytes 2-7 form auto-derived route target see RFC7432 7.6",
                                "interface": [
                                    {"ethernet-interface": f"lag{ _lag_id }"}
                                ],  # Since 22.3.1
    ...
    if state.evpn != "l2_only_leaves":
        updates += [
            (f"/interface[name=irb0]/subinterface[index={_lag_id}]/ipv4/arp", arp)
        ]
    ...
    if state.lacp != "disabled":
        ...
        lag["lag"] = {
            ...
            "lacp": {
                ...
                "lacp-mode": "ACTIVE"
                if leaf_pair_lag
                else state.lacp.upper(),  # ACTIVE or PASSIVE
        ...
        if (
            state.lacp_fallback != 0 and state.pair_role != 2
        ):  # Only enable on one leaf out of any pair
            lag["lag"]["lacp-fallback-mode"] = "static"
            lag["lag"]["lacp-fallback-timeout"] = state.lacp_fallback

    updates += [
        (f"/interface[name=lag{ _lag_id }]", lag),
        (
...
def AddDHCPClient(mac, peer_id, peer_port_list, state, gnmi_client):
    logging.info(
        f"AddDHCPClient :: mac={mac} peer-id={peer_id} peer_port_list={peer_port_list}"
    )
    ...
    lo_ipv4 = {"dhcp-server": {"admin-state": "enable"}}
...
def AddDHCPRelay(port, router_id, state):
    ...
    updates = [
        ("/network-instance[name=evpn-lag-discovery]", {"type": "ip-vrf"}),
        (
            f"/interface[name=irb0]/subinterface[index={_svc_id}]/ipv4/dhcp-relay",
            dhcp_relay,
        ),
    ]
    gnmiConnection(lambda c: c.set_with_retry(encoding="json_ietf", update=updates))
...
##
def Configure_BGP_unnumbered(
    state, port, min_peer_as, max_peer_as, peer_router_id, lag_interface
):
    logging.info(
        f"Configure_BGP_unnumbered :: port={port} peer_router_id={peer_router_id} lag={lag_interface}"
    )
    ...
    if state.igp == "bgp_unnumbered_frr":
        ...
        updates = [
            ("/network-instance[name=default]/protocols/experimental-frr", frr),
            (
                f"/network-instance[name=default]/interface[{eth}.0]/bgp-unnumbered",
                bgp_u,
            ),
            (
        ...
        bgp_group = {
            ...
            "afi-safi": [
                {"afi-safi-name": "evpn", "admin-state": evpn_admin_state},
        ...
        ip_forwarding = {"receive-ipv4-check": False}
        ipv6_ra = {
            "router-advertisement": {"router-role": {"admin-state": "enable"}},
            "admin-state": "enable",
        }

        updates = [
            (
                f"/network-instance[name=default]/protocols/bgp/dynamic-neighbors/interface[interface-{eth}.0]",
                dyn_n,
            ),
            (
                f"/network-instance[name=default]/protocols/bgp/group[group-name={group_name}]",
                bgp_group,
            ),
            (
                "/network-instance[name=default]/protocols/bgp/afi-safi[afi-safi-name=evpn]/evpn",
                bgp_evpn,
            ),
...
def CreateEVPNCommunicationVRF(state, gnmiclient):
    ...
    lldp_rt = (
        f"target:{state.base_as}:65535"  # RT for the cluster AS, EVI 0 doesnt exist
    )
    ...
    if state.evpn_auto_lags == "large_communities":
        ...
        ip_vrf["protocols"]["bgp-vpn"]["bgp-instance"][0]["route-target"][
            "export-rt"
        ] = lldp_rt

    logging.info(f"gNMI SET updates={updates}")
    return gnmiclient.set(encoding="json_ietf", update=updates)
...
def Handle_Notification(obj, state):
    if obj.HasField("config") and obj.config.key.js_path != ".commit.end":
        ...
        if "auto_config" in obj.config.key.js_path:
            ...
            if obj.config.op == 2:
                ...
                if "peerlinks" in data:
                    ...
                    state.peerlinks = list(
                        ipaddress.ip_network(state.peerlinks_prefix).subnets(
                            new_prefix=31
                        )
                    )
                    state.hostlinks_size = int(peerlinks["host_subnet_size"]["value"])
                    state.host_enable_ipv6 = bool(
                        peerlinks["host_enable_ipv6"]["value"]
                    )
                    state.reuse_overlay_ips = bool(
                        peerlinks["reuse_overlay_ips"]["value"]
                    )
                    state.hostlinks = list(
                        ipaddress.ip_network(state.peerlinks_prefix).subnets(
                            new_prefix=state.hostlinks_size
                        )
                    )
                if "loopbacks_prefix" in data:
                    # state.loopbacks = list(ipaddress.ip_network(data['loopbacks_prefix']['value']).subnets(new_prefix=32))
                    state.loopbacks_prefix = ipaddress.ip_network(
                        data["loopbacks_prefix"]["value"]
                    )
                ...
                if "evpn" in data:
                    ...
                    if "auto_lag_ports" in evpn:
                        ...
                        logging.info(
                            f"EVPN auto lag ports: {state.evpn_auto_lag_ports}"
                        )
                ...
                if "igp" in data:
                    ...
                    state.use_bgp_unnumbered = state.igp in [
                        "bgp_unnumbered",
                        "bgp_unnumbered_frr",
                    ]
                if "lacp" in data:
                ...
                state.enable_bfd = (
                    "true"
                    if "enable_bfd" in data and data["enable_bfd"]["value"]
                    else "false"
                )
                ...
                if "gateway" in data:
                    ...
                    state.gateway = {
                        ...
                        "anycast": "anycast_supported" in gw
                        and gw["use_anycast_if_supported"]["value"],
                    }

                # Flag that gets set based on platform feature 'bridged'
                # User can disable it explicitly too, even if supported
                state.bridging_supported = (
                    "bridging_supported" in data and data["bridging_supported"]["value"]
                )
                logging.info(
                    f"Platform supports bridging/mac-vrfs/irb: {state.bridging_supported}"
                )

                state.reload_delay_supported = (
                    "reload_delay_supported" in data
                    and data["reload_delay_supported"]["value"]
                )
                reload_delay = (
                    int(data["reload_delay_secs"]["value"])
                    if "reload_delay_secs" in data
                    else 0
                )
                state.reload_delay = (
                    {"reload-delay": reload_delay}
                    if state.reload_delay_supported and reload_delay > 0
                    else {}
                )
                logging.info(
                    f"Platform supports reload-delay: {state.reload_delay_supported} secs={reload_delay}"
                )
        ...
        if my_port != "mgmt0" and to_port != "mgmt0" and hasattr(state, "peerlinks"):
            logging.info(
                f"process LLDP notification on port {my_port}: op='{obj.lldp_neighbor.op}' peer='{peer_sys_name}'"
            )
            ...
            if m and not re.match("^h[0-9]+", peer_sys_name):
                ...
                to_port_id = (
                    my_port_id  # FRR Linux host or other element not sending port name
                )
                if not state.host_lldp_seen and state.role == "auto":
                    logging.info(
                        "Received host LLDP (no portname match); auto switching to leaf"
                    )
            ...
            def delay_config():
                state.pending_peers[my_port] = (
                    int(my_port_id),
                    int(to_port_id),
                    peer_sys_name,
                    obj.lldp_neighbor.key.chassis_id,
                    obj.lldp_neighbor.data.port_id
                    if obj.lldp_neighbor.key.chassis_id
                    != obj.lldp_neighbor.data.port_id
                    else None,
                    desc,
                )
            ...
            if not hasattr(state, "node_id"):
                node_id = determine_local_node_id(
                    state, int(my_port_id), int(to_port_id), peer_sys_name
                )
                if node_id == 0:
                    return delay_config()
            ...
            logging.info(
                f"Handle_Notification: m={m} has_router_id={hasattr(state,'router_id')}"
            )
            if m and not hasattr(
                state, "router_id"
            ):  # Only for valid to_port, if not set
                state.router_id = state.determine_router_id(
                    state.get_role(), state.node_id
                )
            ...
            if router_id_changed:
                for intf in state.pending_peers:
                    (
                        ...
                        _lldp_port,
                        _lldp_desc,
                    ) = state.pending_peers[intf]
                    _is_lag = configure_peer_link(
                        state,
                        intf,
                        _my_port_id,
                        _to_port_id,
                ...
                if state.get_role() == "leaf":
                    ...
                    EVPNRouteMonitoringThread(
                        state
                    ).start()  # Needed for populating DHCP

            # Could also announce communities for spines
            if (
                lag_created
            ):  # state.get_role() == "leaf" and hasattr(state,"router_id"):
                Announce_LLDP_using_EVPN(
                    state, obj.lldp_neighbor.key.chassis_id, [int(my_port_id)]
                )
                if (
                    state.dhcp
                    and obj.lldp_neighbor.key.chassis_id
                    != obj.lldp_neighbor.data.port_id
                ):
                    # sets flag bit
                    Announce_LLDP_using_EVPN(
                        state, obj.lldp_neighbor.data.port_id, [int(my_port_id)], True
                    )
            else:
                logging.info(
                    f"Not (yet) creating LLDP Community for port {my_port_id} peer={peer_sys_name}"
                )
...
def determine_local_node_id(state, lldp_my_port, lldp_peer_port, lldp_peer_name):
    ...
    if state.is_spine():
        ...
        if leafId:
            ...
            return (
                int(leafId.groups()[0]) - 1
            ) * state.max_hosts_per_leaf + lldp_peer_port
        return lldp_peer_port


def configure_peer_link(
    state,
    intf_name,
    lldp_my_port,
    lldp_peer_port,
    ...
    if state.is_spine():  # (super)spines
        ...
        # state.max_spine_ports default = 8
        # XXX this assumes ports are used starting at index 1, not 49
        if lldp_my_port > state.max_spine_ports:
            logging.error(
                f"max-spine-ports configured too low({state.max_spine_ports}), will result in duplicate link IPs"
            )
            return False
        ...
        logging.info(
            f"Configure SPINE port towards {peer_type}: link_index={link_index}[{_r}] local_port={lldp_my_port} peer_port={lldp_peer_port}"
        )
        ...
        if spineId:  # For spine facing links, pick based on peer_port
            ...
            link_index = state.max_spine_ports * (node_id - 1) + lldp_peer_port - 1
            peer_type = "spine"
            peer_router_id = state.determine_router_id(
                peer_type, int(spineId.groups()[0])
            )
            ...
            if leaf_pair:
                ...
                if state.pair_role != 0 and peer_role and same_pair:
                    if (state.pair_role == 1 and peer_role == "a") or (
                        state.pair_role == 2 and peer_role == "b"
                    ):
                        logging.info(
                            f"Detected leaf self loop on ports {lldp_my_port} and {lldp_peer_port}"
                        )
                ...
                min_peer_as = max_peer_as = (
                    state.host_as if state.host_as != 0 else state.evpn_overlay_as
                )
        ...
        logging.info(
            f"Configure LEAF port towards {peer_type}: link_index={link_index} local_port={lldp_my_port} peer_port={lldp_peer_port} peer_router_id={peer_router_id} min_peer_as={min_peer_as}"
        )
    else:  # Emulated Hosts
        _r = 1
    ...
    if not hasattr(state, link_name):
        if (
            not state.use_bgp_unnumbered
            or peer_type == "host"
            or leaf_pair_link
            or state.role == "endpoint"
        ):
            ...
            _peer = "*"
        logging.info(
            f"Configuring link {link_name} local_port={lldp_my_port} peer_port={lldp_peer_port} ip={_ip} peer={_peer}"
        )
        script_update_interface(
            ...
            lldp_peer_desc,
            _peer
            if ((peer_type == "spine" and _r == 1) or leaf_pair_link)
            else "*",  # Only when connecting "upwards"
        ...
        lag_interface = None
        if (
            (peer_type == "host" and state.host_use_irb)
            or (
                state.evpn == "l2_only_leaves"
                and (state.get_role(), peer_type)
                in [("spine", "leaf"), ("leaf", "spine"), ("leaf", "leaf")]
            )
            ...
            logging.info(
                f"Not a host/l2 leaf facing port ({peer_type}) or configured to not use IRB: {intf_name}"
            )

        if peer_type == "host" and state.evpn != "disabled":
            Configure_EVPN(
                state, lldp_my_port, lag_interface or f"ethernet-1/{lldp_my_port}", _ip
            )
        ...
        logging.info(
            f"Link {link_name} already configured local_port={lldp_my_port} peer_port={lldp_peer_port}"
        )
        return False
                    ...
                    "subinterface": [
                        {
                            ...
                            "admin-state": "enable",
                            "ipv4": {
                                "address": [{"ip-prefix": LOOPBACK_IP4}],
                                "admin-state": "enable",
                            },
                            "ipv6": {
                                "address": [{"ip-prefix": LOOPBACK_IP6}],
                                "admin-state": "enable",
                            },
                        }
                    ],
...
class State(object):
    ...
    def _determine_role(self):
        ...
        role_id = re.match(
            "^(\w+)[-]?(\d+)(a|b)?.*$", hostname
        )  # Ignore trailing router ID, if set
        if role_id:
            ...
            logging.info(
                f"_determine_role: role={self.role} pair_role={self.pair_role} id={self.id_from_hostname}"
            )

            # TODO super<n>spine
        else:
            logging.warning(
                f"_determine_role: Unable to determine role/id based on hostname: {hostname}, switching to 'auto' mode"
            )
            self.role = "auto"
            self.pair_role = 0
        ...
        if m:
            ...
            if peer_level > self.node_level():
                if peer_max_level > self.max_level:
                    ...
                    self.top_count = max(
                        peer_top_count, peer_id
                    )  # Reset to peer's count
                    logging.info(
                        f"self.max_level updated to {peer_max_level}, top_count reset to {self.top_count} based on LLDP peer name '{peer_lldp_sysname}'"
                    )
                    change = True
                elif peer_top_count > self.top_count or (
                    peer_level == self.max_level and peer_id > self.top_count
                ):
                    self.top_count = max(peer_top_count, peer_id)
                    logging.info(
                        f"self.top_count updated to {self.top_count} based on LLDP peer name '{peer_lldp_sysname}'"
                    )
                    change = True
        else:
            ...
            if m:
                ...
                if m.groups()[2]:
                    ...
                    logging.info(
                        f"update_max_level: detected leaf-pair peer -> effective id {peer_id}"
                    )
    ...
    def commit(self):
        ...
        for evi, s in self.services.items():
            ...
            if not is_l3 and not self.bridging_supported:
                logging.warning(
                    f"mac-vrf not supported on this platform, skipping: {s}"
                )
                continue
            svc = {"type": "ip-vrf" if is_l3 else "mac-vrf"}
            updates += [(f"/network-instance[name={svc_name}]", svc)]
            s["created"] = svc
        if updates != []:
            logging.info(f"About to create: {updates}")
            gnmiConnection(
                lambda c: c.set_with_retry(encoding="json_ietf", update=updates)
            )
